...
{
    //스크립트 내에서 이벤트 함수를 작성하고, 내부에 진행할 명령문을 작성하면 상황에 맞는 해당 기능이 수행됩니다.
    private void Awake()
    {
        Debug.Log("[Awake]");
        ...
        Debug.Log("각 스크립트 기준 한 번만 호출이 되고 다른 개체의 초기화가 완료 된 후 호출되는 영역이기 때문에 다른 컴포넌트에 대한 참조를 만들어야하는 경우 이 위치에서 만들면 안전하게 처리됩니다.");
        //코루틴(Coroutine) : 코드를 일시 중지하고 특정 조건이 충족될 때까지 실행을 delay시킬 수 있는 기능.(ex. 3초 뒤에 오브젝트를 파괴합니다.)
    }
    ...
    void Update()
    {
     //화면에 랜더링되는 주기가 1초에 약 60번 정도 호출됩니다. (하드웨어 성능에 따라 차이가 날 수 있습니다.)

        //Time.deltaTime을 통해 이전 프레임 ~ 현재 프레임까지의 시간차이로 보정 값을 주거나
        //정규화 / 단위 벡터를 이용해 작업을 처리합니다.
        //기본적으로 계산에 보정 값들이 많이 사용됩니다.

        //프로그램 내에서 핵심적으로 계속 사용되는 메인 로직을 짜는 위치로 사용됩니다.
        // ex) 키입력 등을 기반으로 움직임(지속적인 업데이트가 요구되는 경우)
        ...
        //업데이트는 써야하는 기능이고, 가장 많이 사용되는 영역.
        //따라서 업데이트에서 무조건 실행돼야하는 상황이 아니라면 다른 영역에서 작업을 하게 설계하는 것이
    ...
    {
        //모든 Update 함수(Update, FixedUPdate)가 호출된 다음에 마지막으로호출되는 영역입니다.
        //후처리 작업에 사용됩니다.
